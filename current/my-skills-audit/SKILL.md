...
`<repo>/current/my-skills-audit/SKILL.md`, so:

- `<repo>` = this file's directory, two levels up
- checks script = `<repo>/current/my-skills-audit/scripts/mechanical-checks.py`
...
Resolve `<repo>` as above. Confirm `<repo>/current/` exists and contains at least one
...
compare its keys against `<repo>/current/*/`. A key with no corresponding directory means a
