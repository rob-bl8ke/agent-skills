...
## Cross-shell GitHub writes

- Keep the workflow shell-neutral until the moment you actually need to run `gh`. Drafting, breakdown, and review are identical across shells.
- Before any create or edit, consult `references/shell-selection.md` to classify the active shell path as PowerShell or bash/zsh. Do not mix syntax between them.
- After shell selection, follow exactly one write-path reference:
   - PowerShell: `references/github-cli-powershell.md`
   - bash/zsh: `references/github-cli-bash.md`
- If the shell cannot be identified confidently, stop the write path and return the approved draft plus the matching manual command shape from the relevant reference. Do not guess.
...
- Preserve the approved body exactly. Never paste multi-line issue text directly into executable command syntax; write a UTF-8 temporary payload file and pass it with `--body-file`.
- Immediately verify any successful create or edit with a fresh `gh issue view` call and report the exact verified issue number and URL back to the user.

**Why this matters:** the recurring failure mode is not GitHub itself; it is shell syntax leakage. A shell-neutral main flow plus an explicit per-shell write path prevents Bash redirection patterns from being emitted in PowerShell and vice versa.
...
- Do not emit a Bash heredoc, `printf` redirection, or command substitution while operating in PowerShell.
- Do not emit PowerShell `try`/`finally`, `Remove-Item`, or `$LASTEXITCODE` checks while operating in bash/zsh.
- Do not treat "Windows" as meaning PowerShell or "Linux/macOS" as meaning bash. The active runner shell decides the command shape.
- Do not inline the full issue body into a single shell command argument when `--body-file` is available.
- Do not retry `gh issue create` after an ambiguous failure until you have reconciled with `gh issue view` or `gh issue list` to avoid duplicates.
